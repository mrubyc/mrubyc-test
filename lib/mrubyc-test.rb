      ...
      begin
        ...
        Dir.chdir(tmp_dir) do
          [
           ...
           "ls -l",
           "cc -v -Wl,--no-as-needed -lm -I #{pwd}/#{config['mrubyc_src_dir']} -DMRBC_DEBUG -DMRBC_USE_MATH=1 -o test main.c #{pwd}/#{config['mrubyc_src_dir']}/*.c #{pwd}/#{config['mrubyc_src_dir']}/hal/*.c",
           "ls -l",
           './test'].each do |cmd|
             puts cmd
             puts
